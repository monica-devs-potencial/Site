...
if (isAdmin()) {
    $stmt = $db->prepare("UPDATE messages SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL");
    $stmt->execute([$id]);
} else {
    $stmt = $db->prepare("UPDATE messages SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ? AND deleted_at IS NULL");
